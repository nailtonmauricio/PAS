...
if (!empty($id)) {
    ...
    $sql = "SELECT pal.al_id, pal.access FROM page_access_level AS pal WHERE id =:id LIMIT 1";
    ...
    if ($res ->rowCount()) {
        ...
        $sql_update = "UPDATE page_access_level AS pal SET pal.access =:permissao, modified = CURRENT_TIMESTAMP WHERE pal.id =:id";
        ...
        if($res_update ->rowCount()){
            echo $url_destino = pg . "/list/list_permissoes?id={$row["al_id"]}";
        }
        if ($res_update ->rowCount()) {
